...
//   · DRCCLACBS (credit card delinquency, 5% CSC weight) added to MACRO_SERIES;
//     quarterly release → often null → defaults to score 25.
//
// Sync fixes applied 2026-06-26 (bugs vs ic-compute.js):
//   · WRESBAL: was divided by 1e6 (wrong); FRED WRESBAL is billions → fixed to /1000.
//   · M2 annualized: was using raw MoM changePct; fixed to ×12 (same as ic-compute.js).
//   · CSC weights: hyOas weight corrected 0.30→0.25; DRCCLACBS (0.05) added.
//   · MOVE blend: was 50/50 average; corrected to 70/30 (ic-compute.js formula).
//   · SOFR fallback: was ((5.5-sofr)/5.5)*100; corrected to 50+(4.0-sofr)*12.5.
...
const MACRO_SERIES = [
  ...
  // CSC
  "BAMLH0A0HYM2", "BAMLH0A1HYBB", "BAMLH0A3HYC", "BAMLC0A4CBBB", "STLFSI4", "DRTSCILM", "DRCCLACBS",
  // RPC
...
function computeCompositeScores(ds, polyData = null) {
  ...
  const lccCalc = (() => {
    ...
    const wresbalT = wresbalB != null ? wresbalB / 1000 : null; // FRED WRESBAL in billions → T
    ...
    if (sofr != null && fedfunds != null) {
      ...
      sofr_score = Math.max(0, Math.min(100, 50 + (4.0 - sofr) * 12.5));
    }
    const m2chgRaw = ds?.["M2SL"]?.changePct ?? null;
    const m2chg = m2chgRaw != null ? m2chgRaw * 12 : null; // MoM → annualized (thresholds are YoY)
    let m2_score = 50;
    if (m2chg != null) {
  ...
  const cscCalc = (() => {
    ...
    if (moveVal != null) {
      ...
      if (moveStress > hyRoC_score) hyRoC_score = hyRoC_score * 0.70 + moveStress * 0.30;
    }
    ...
    const drcclacbs = safe("DRCCLACBS");
    let drcclacbs_score = 25;
    if (drcclacbs != null) {
      drcclacbs_score = Math.max(0, Math.min(100,
        drcclacbs < 2.5 ? 5
        : drcclacbs < 4 ? 5 + ((drcclacbs - 2.5) / 1.5) * 30
        : drcclacbs < 6 ? 35 + ((drcclacbs - 4) / 2) * 40
        : 75 + Math.min(25, ((drcclacbs - 6) / 1.5) * 25)));
    }
    const csc =
      hyOas_score * 0.25 + hyRoC_score * 0.20 + bbCcc_score * 0.15 + bbb_score * 0.15 + stlfsi_score * 0.10 + candi_score * 0.10 + drcclacbs_score * 0.05;
...
async function buildResearchIndicators(fredKey) {
  const out = {
    ...
    boj_assets: null, m2_growth_yoy: null, claims_trend: null, profits_trend: null,
    ...
    // P3: new FRED series (Tier B — extends Research layer)
    dgs1: null, dgs5: null,
    payems: null, breakeven_10y: null, nfci: null,
    median_home_price_chg: null,
    // P4: Core CPI
    core_cpi_yoy: null,
    // Phase 5: TED spread input + MOVE proxy; Phase 6: skew (computed in buildMacroState)
    dtb3: null, move_index: null,
  };
  ...
  const [dgs2, dgs10, dgs30, tp10, icsa, cp, walcl, rrp, tga, boj,
         pce, unrate, wti, ncbeil, gdpArr, ecb,
         dgs1, dgs5, payems, t10yie, nfci, mspus, cpiCore, dtb3, m2yoy] = await Promise.all([
    tryGet("DGS2", 70), tryGet("DGS10", 25), tryGet("DGS30", 70), // DGS10: 25 obs for MOVE proxy
    ...
    tryGet("ECBASSETSW", 6),
    // P3: new series (graceful — never break regime)
    tryGet("DGS1", 5), tryGet("DGS5", 5),
    tryGet("PAYEMS", 2),
    tryGet("T10YIE", 5),
    tryGet("NFCI", 5),
    tryGet("MSPUS", 8),
    // P4: Core CPI YoY (CPILFESL = CPI ex food & energy)
    tryGet("CPILFESL", 2, "pc1"),
    // Phase 5: DTB3 (3-Month T-Bill) for TED spread
    tryGet("DTB3", 5),
    // M2 YoY % (pc1) for the research display — the liquidity composite keeps its
    // own MoM×12 proxy; this is the sensible headline number (true YoY, not annualized MoM).
    tryGet("M2SL", 2, "pc1"),
  ]);
  ...
  out.m2_growth_yoy = m2yoy?.[0]?.v ?? null;
  ...
  if (wti && wti.length > 21) {
    ...
    if (mAgo) {
      ...
      out.oil_shock = out.wti_chg_1m != null
      ? (out.wti_chg_1m >= 15 ? 'shock ↑' : out.wti_chg_1m <= -15 ? 'shock ↓' : null)
      : null;
    }
  }
  ...
  // P3: new FRED series (written directly from fresh fetches above)
  out.dgs1          = dgs1?.[0]?.v ?? null;
  out.dgs5          = dgs5?.[0]?.v ?? null;
  out.payems        = payems?.[0]?.v ?? null;        // thousands of persons (NFP)
  out.breakeven_10y = t10yie?.[0]?.v ?? null;        // T10YIE %
  out.nfci          = nfci?.[0]?.v ?? null;
  if (mspus && mspus.length >= 5) {
    const now = mspus[0].v, yearAgo = mspus[4].v;
    if (yearAgo) out.median_home_price_chg = +((now - yearAgo) / yearAgo * 100).toFixed(1);
  }
  out.core_cpi_yoy  = cpiCore?.[0]?.v != null ? +cpiCore[0].v.toFixed(2) : null;

  // Phase 5 — DTB3 level (used for TED spread in buildMacroState)
  out.dtb3 = dtb3?.[0]?.v ?? null;

  // Phase 5 — MOVE proxy: annualised std-dev of daily DGS10 changes (in bps)
  // DGS10 obs sorted newest-first; reverse to oldest-first before computing diffs.
  // std(daily_Δ in %-pts) × √252 × 100 → basis points/year ≈ MOVE index
  if (dgs10 && dgs10.length >= 3) {
    const obs = dgs10.slice(0, Math.min(23, dgs10.length)).reverse(); // oldest-first
    const changes = [];
    for (let i = 1; i < obs.length; i++) changes.push(obs[i].v - obs[i - 1].v);
    if (changes.length >= 2) {
      const mean = changes.reduce((s, v) => s + v, 0) / changes.length;
      const variance = changes.reduce((s, v) => s + (v - mean) ** 2, 0) / changes.length;
      out.move_index = +(Math.sqrt(variance) * Math.sqrt(252) * 100).toFixed(1);
    }
  }
...
async function buildCreditProxy(finnhubKey) {
  if (!finnhubKey) return { credit_private_proxy: null, credit_divergence: null, gold_price: null };
  const q = async (sym) => {
    try {
  ...
  const [bizd, bkln, gld] = await Promise.all([q("BIZD"), q("BKLN"), q("GLD")]);
  const ch = [bizd?.dp, bkln?.dp].filter((v) => typeof v === "number");
  const proxy = ch.length ? ch.reduce((s, v) => s + v, 0) / ch.length : null;
  const goldPrice = gld?.c != null ? +Number(gld.c).toFixed(2) : null;
  return { credit_private_proxy: proxy != null ? +proxy.toFixed(3) : null, credit_divergence: null, gold_price: goldPrice };
}
...
// ── Dalio Long-Term Debt Cycle stage (1–5) ───────────────────────────────────
// Derives the structural US debt-cycle stage from available FRED data.
// The US has been in Stage 4 (Political Fracture) since ~2020: interest payments
// crossed defense spending in 2024, ~18% of federal revenue (CBO). The structural
// anchors (not on FRED, updated quarterly from BIS/CBO) are encoded as comments.
//
//   Stage 1: Healthy accumulation    — pre-GFC, low debt/GDP, small balance sheet
//   Stage 2: Currency as Tool        — ZIRP + QE era (2009-2015, 2020-2021)
//   Stage 3: Creditors Diversify     — rates rising, foreign trimming (2018-2019, 2022)
//   Stage 4: Political Fracture      — current US (2026): interest > defense, ~18% revenue
//   Stage 5: Extraordinary Measures  — monetization crisis (not yet; requires extreme triggers)
//
// Inputs: ds (FRED data store), research (buildResearchIndicators output), scores
// Returns: integer 1–5
function computeDalioStage(ds, research, scores) {
  const walclT  = ds?.WALCL?.value  != null ? ds.WALCL.value / 1e6 : null;  // Fed BS in $T
  const dgs10   = research.dgs10    ?? null;  // 10Y Treasury %
  const fedfunds = ds?.DFEDTARL?.value ?? null; // Fed funds target %
  const m2Gr    = ds?.M2SL?.changePct != null ? ds.M2SL.changePct * 12 : null; // M2 annualized %
  const dxy     = ds?.DTWEXBGS?.value  ?? null;  // USD broad trade-weighted
  const csc     = scores.creditStress;           // 0-100, higher = more stress

  // Stage 5 — all four crisis triggers must fire simultaneously to avoid false positives.
  // Threshold calibration: $10T BS (well above today's ~$6.7T), 15% M2 (pandemic peak was ~27%),
  // CSC >70 (crisis zone), DXY <88 (below 2021 low of ~89, signalling reserve-currency fracture).
  if (
    walclT  != null && walclT  > 10 &&
    m2Gr    != null && m2Gr    > 15 &&
    csc     > 70 &&
    dxy     != null && dxy     < 88
  ) return 5;

  // Stage 4 — US structural position since ~2020 (Political Fracture).
  // FRED-observable proxy: large balance sheet (>$5T = post-GFC new normal persisting) AND
  // elevated long rates (>3% = meaningful debt-servicing burden at current debt/GDP).
  // Structural fact (BIS/CBO, quarterly — not on FRED daily):
  //   • Federal interest payments ≈ $1T+, ~18% of federal revenue (reviewed quarterly)
  //   • Interest outpaced defense spending in FY2024
  if (walclT != null && walclT > 5 && dgs10 != null && dgs10 > 3.0) return 4;

  // Stage 3 — Creditors Diversify: rates rising but balance sheet not yet overextended.
  // Describes 2018-2019 QT, or 2022-2023 early tightening cycle.
  if (dgs10 != null && dgs10 > 2.5 && fedfunds != null && fedfunds > 1.5) return 3;

  // Stage 2 — Currency as Tool: ZIRP + active QE (2009-2015, 2020-2021 pandemic).
  if (fedfunds != null && fedfunds < 1.0 && walclT != null && walclT > 3) return 2;

  // Stage 1 — Accumulation: pre-GFC healthy debt dynamics (rare in modern context).
  return 1;
}

// ════════════════════════════════════════════════════════════════════════════
// STATIONARY RISK-ON GAUGE (Phase 1 validated) — the multi-asset allocator's input.
// Stationarizes 3 drivers onto a comparable percentile scale (rank vs trailing 60m)
// so the gauge is regime-relative and historically valid, unlike the level-calibrated
// production composites:
//   LCC = 6-month net-liquidity impulse (WALCL−WTREGEN−RRP, $T), percentile-ranked
//   RPC = 0.5·pctile(−T10Y3M inverted curve) + 0.5·pctile(SAHMREALTIME)
//   CSC = pctile(STLFSI4)
//   riskOn = 0.5·LCC + 0.25·(100−RPC) + 0.25·(100−CSC)
// MIRRORS scora-research/research/regimeStationary.mjs EXACTLY (validated OOS 2007-26:
// RiskOn tilt + dual-momentum → Sharpe 1.01). ADDITIVE & nullable: any failure leaves
// risk_on null and never touches the regime upsert.
// ════════════════════════════════════════════════════════════════════════════
const RISKON_SERIES = ["WALCL", "WTREGEN", "RRPONTSYD", "T10Y3M", "SAHMREALTIME", "STLFSI4"];
const RISKON_LOOKBACK = 60, RISKON_IMPULSE = 6, RISKON_GRID_START = "2003-01-01";

async function fetchFredHistory(seriesId, fredKey) {
  const url = `https://api.stlouisfed.org/fred/series/observations?series_id=${seriesId}&api_key=${fredKey}&file_type=json&observation_start=2000-01-01`;
  const r = await fetch(url);
  if (!r.ok) throw new Error(`FRED ${seriesId} HTTP ${r.status}`);
  const d = await r.json();
...
function _rkAsOf(obs, date) {
  let i = -1;
  for (let lo = 0, hi = obs.length - 1; lo <= hi;) { const m = (lo + hi) >> 1; if (obs[m].date <= date) { i = m; lo = m + 1; } else hi = m - 1; }
  return i < 0 ? null : obs[i].v;
}
function _rkAddMonths(dstr, n) { const x = new Date(dstr + "T00:00:00Z"); x.setUTCMonth(x.getUTCMonth() + n); return x.toISOString().slice(0, 10); }
// percentile rank (0-100) of the last non-null value vs the trailing RISKON_LOOKBACK window
function _rkPctileLast(arr) {
  let i = arr.length - 1; while (i >= 0 && arr[i] == null) i--;
  if (i < 0) return null;
  const cur = arr[i]; const hist = [];
  for (let j = Math.max(0, i - RISKON_LOOKBACK); j <= i; j++) if (arr[j] != null) hist.push(arr[j]);
  if (hist.length < 12) return null;
  return (hist.filter((x) => x <= cur).length / hist.length) * 100;
}

// Returns { risk_on, risk_on_lcc, risk_on_rpc, risk_on_csc } (all 0-100) or nulls.
async function buildStationaryRiskOn(fredKey) {
  const hist = {};
  await Promise.all(RISKON_SERIES.map(async (id) => { hist[id] = await fetchFredHistory(id, fredKey); }));
  if (!hist.WALCL || !hist.WALCL.length) return { risk_on: null };
  const end = hist.WALCL[hist.WALCL.length - 1].date;
  const grid = []; for (let d = RISKON_GRID_START; d <= end; d = _rkAddMonths(d, 1)) grid.push(d);
  const sample = (id) => grid.map((dt) => _rkAsOf(hist[id] || [], dt));
  const walcl = sample("WALCL"), wt = sample("WTREGEN"), rr = sample("RRPONTSYD");
  const netliq = grid.map((_, i) => walcl[i] == null ? null : (walcl[i] - (wt[i] ?? 0)) / 1e6 - (rr[i] ?? 0) / 1000);
  const liqImpulse = grid.map((_, i) => (i >= RISKON_IMPULSE && netliq[i] != null && netliq[i - RISKON_IMPULSE] != null) ? netliq[i] - netliq[i - RISKON_IMPULSE] : null);
  const negCurve = sample("T10Y3M").map((v) => (v == null ? null : -v));
  const lcc = _rkPctileLast(liqImpulse);
  const rc = _rkPctileLast(negCurve), rs = _rkPctileLast(sample("SAHMREALTIME"));
  const rpc = rc != null && rs != null ? 0.5 * rc + 0.5 * rs : (rc ?? rs);
  const csc = _rkPctileLast(sample("STLFSI4"));
  if (lcc == null || rpc == null || csc == null) return { risk_on: null };
  const riskOn = Math.max(0, Math.min(100, 0.5 * lcc + 0.25 * (100 - rpc) + 0.25 * (100 - csc)));
  return { risk_on: +riskOn.toFixed(1), risk_on_lcc: +lcc.toFixed(1), risk_on_rpc: +rpc.toFixed(1), risk_on_csc: +csc.toFixed(1) };
}

// CBOE 3-month implied correlation (^COR3M) — the live "stock-picking regime" input for
// the momentum micro score (Phase 4). Low = dispersion (momentum selection pays); high =
// macro tape (momentum crashes). Free via Yahoo. Additive & nullable; graceful on failure.
async function buildImpliedCorr() {
  try {
    const url = `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent("^COR3M")}?range=10d&interval=1d`;
    const r = await fetch(url, { headers: { "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/124.0 Safari/537.36" } });
    if (!r.ok) throw new Error(`Yahoo COR3M HTTP ${r.status}`);
    const res = (await r.json())?.chart?.result?.[0];
    const closes = res?.indicators?.quote?.[0]?.close || [];
    for (let i = closes.length - 1; i >= 0; i--) if (closes[i] != null) return { implied_corr: +Number(closes[i]).toFixed(1) };
    return { implied_corr: null };
  } catch { return { implied_corr: null }; }
}

// Shiller CAPE (secular valuation, Phase 5) — best-effort scrape of multpl.com. The
// robust secular gauge is the Buffett Indicator (from FRED, already computed); CAPE is
// a complementary reference. Additive & nullable; graceful on failure.
async function buildSecularCape() {
  try {
    const r = await fetch("https://www.multpl.com/shiller-pe", { headers: { "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/124.0 Safari/537.36" } });
    if (!r.ok) throw new Error(`multpl HTTP ${r.status}`);
    const t = await r.text();
    const m = t.match(/Current Shiller PE Ratio[:\s]*([0-9]{1,2}\.[0-9]{1,2})/i) || t.match(/Shiller PE[^0-9]{0,40}([0-9]{2}\.[0-9]{1,2})/i);
    const v = m ? parseFloat(m[1]) : null;
    return { cape: v != null && v > 5 && v < 100 ? +v.toFixed(1) : null };
  } catch { return { cape: null }; }
}
...
async function buildMacroState(fredKey, opts = {}) {
  ...
  let riskOn = { risk_on: null };
  try { riskOn = await buildStationaryRiskOn(fredKey); }
  catch (e) { errors.push(`risk_on: ${e.message}`); }
  let corr = { implied_corr: null };
  try { corr = await buildImpliedCorr(); }
  catch (e) { errors.push(`implied_corr: ${e.message}`); }
  let secular = { cape: null };
  try { secular = await buildSecularCape(); }
  catch (e) { errors.push(`cape: ${e.message}`); }
  ...
  // Phase 5 — TED spread (post-LIBOR, in basis points)
  // = (SOFR + ARRC_adjustment) − DTB3   [ARRC spread: 0.26161 % = 26.161 bps]
  const sofrVal = ds?.SOFR?.value ?? null;
  const dtb3Val = research.dtb3 ?? null;
  const ted_spread = sofrVal != null && dtb3Val != null
    ? +((sofrVal + 0.26161 - dtb3Val) * 100).toFixed(1)
    : null;

  // Phase 6 — SKEW proxy from VIX (VIX=15 → SKEW≈100; calibrated to CBOE range)
  const vixVal = ds?.VIXCLS?.value ?? null;
  const skew_index = vixVal != null ? +(100 + (vixVal - 15) * 1.2).toFixed(1) : null;
  ...
  // ic_score: overall macro backdrop for equities (0=hostile, 100=ideal)
  const icScoreVal = scores.liquidityCycle * 0.40
    + (100 - scores.recessionProbability) * 0.35
    + (100 - scores.creditStress) * 0.25;

  // meltup_score: euphoria/speculation gauge (higher = more meltup risk)
  const fgVal    = sentiment.fear_greed ?? 50;
  const m2Val    = (ds?.M2SL?.changePct != null ? ds.M2SL.changePct * 12 : null) ?? 0;
  const buffVal  = research.buffett_indicator ?? 120;
  const pcVal    = sentiment.put_call_ratio ?? 1.0;
  let meltup = 50 + (fgVal - 50) * 0.40;
  if (m2Val > 6)       meltup += Math.min(15, (m2Val - 6) * 3);
  if (buffVal > 150)   meltup += Math.min(15, (buffVal - 150) * 0.25);
  if (pcVal < 0.8)     meltup += Math.min(10, (0.8 - pcVal) * 25);

  // bubble_debt: debt-driven bubble risk
  let bubDebt = 30;
  if (buffVal > 130)   bubDebt += Math.min(40, (buffVal - 130) / 70 * 40);
  if (scores.creditStress > 40) bubDebt += Math.min(20, (scores.creditStress - 40) / 60 * 20);
  const hyOasVal = ds?.BAMLH0A0HYM2?.value ?? null;
  if (hyOasVal != null && hyOasVal > 350) bubDebt += Math.min(10, (hyOasVal - 350) / 150 * 10);

  // bubble_ai: AI/tech speculation bubble risk
  let bubAI = 30;
  if (scores.liquidityCycle > 60) bubAI += Math.min(20, (scores.liquidityCycle - 60) / 40 * 20);
  if (buffVal > 150)   bubAI += Math.min(25, (buffVal - 150) / 50 * 25);
  if (fgVal > 60)      bubAI += Math.min(15, (fgVal - 60) / 40 * 15);
  if (scores.creditStress < 30) bubAI += 10;

  // Dalio stage — derived from FRED proxies; graceful (never throws).
  let dalio_stage = null;
  try { dalio_stage = computeDalioStage(ds, research, scores); } catch { /* leave null */ }
  ...
  const row = {
    ...
    risk_on: riskOn.risk_on ?? null,   // stationary liquidity-led gauge for the allocator (additive, nullable)
    risk_on_lcc: riskOn.risk_on_lcc ?? null, // driver: liquidity-impulse percentile
    risk_on_rpc: riskOn.risk_on_rpc ?? null, // driver: recession-risk percentile
    risk_on_csc: riskOn.risk_on_csc ?? null, // driver: financial-stress percentile
    implied_corr: corr.implied_corr ?? null, // ^COR3M — stock-picking regime (Phase 4)
    cape: secular.cape ?? null,              // Shiller CAPE — secular valuation (Phase 5)
    regime_id: reg ? reg.id : null,
    regime_label: reg ? reg.label : null,
    cartera_quadrant: (reg && QMAP[reg.id]) || "defensivo",
    updated_at: new Date().toISOString(),
    ic_score: +Math.max(0, Math.min(100, icScoreVal)).toFixed(1),
    meltup_score: +Math.max(0, Math.min(100, meltup)).toFixed(1),
    bubble_debt: +Math.max(0, Math.min(100, bubDebt)).toFixed(1),
    bubble_ai: +Math.max(0, Math.min(100, bubAI)).toFixed(1),
    hy_oas_momentum: ds?.BAMLH0A0HYM2?.change != null ? +ds.BAMLH0A0HYM2.change.toFixed(1) : null,
    ...
    // ── P1: Composite input series — already fetched by MACRO_SERIES, now written to row ──
    // Unit conversions (FRED_UNIT_CONVERSIONS) are applied inside fetchFredObs before ds is built,
    // so .value here is already in model units: OAS spreads in bps, ICSA in K, etc.
    hy_oas:             ds?.BAMLH0A0HYM2?.value ?? null,   // bps
    hy_bb_oas:          ds?.BAMLH0A1HYBB?.value ?? null,   // bps
    hy_ccc_oas:         ds?.BAMLH0A3HYC?.value ?? null,    // bps
    bbb_oas:            ds?.BAMLC0A4CBBB?.value ?? null,   // bps
    stlfsi4:            ds?.STLFSI4?.value ?? null,
    c_and_i_loans:      ds?.DRTSCILM?.value ?? null,
    credit_card_delinq: ds?.DRCCLACBS?.value ?? null,
    t10y2y:             ds?.T10Y2Y?.value ?? null,          // %
    t10y3m:             ds?.T10Y3M?.value ?? null,          // %
    sahm_rule:          ds?.SAHMREALTIME?.value ?? null,
    icsa:               ds?.ICSA?.value ?? null,            // K (×0.001 applied)
    umcsent:            ds?.UMCSENT?.value ?? null,
    vix:                ds?.VIXCLS?.value ?? null,
    ovx:                ds?.OVXCLS?.value ?? null,
    brent:              ds?.DCOILBRENTEU?.value ?? null,    // $/bbl
    dxy:                ds?.DTWEXBGS?.value ?? null,
    usdjpy:             ds?.DEXJPUS?.value ?? null,
    m2_growth:          research.m2_growth_yoy ?? (ds?.M2SL?.changePct != null ? ds.M2SL.changePct * 12 : null),  // true YoY (pc1), MoM×12 proxy fallback
    walcl:              ds?.WALCL?.value ?? null,           // $M
    rrpontsyd:          ds?.RRPONTSYD?.value ?? null,       // $B
    wresbal:            ds?.WRESBAL?.value ?? null,         // $B
    sofr:               ds?.SOFR?.value ?? null,            // %
    fedfunds:           ds?.DFEDTARL?.value ?? null,        // %
    mortgage_rate:      ds?.MORTGAGE30US?.value ?? null,    // %
    home_sales:         ds?.EXHOSLUSM495S?.value ?? null,   // K SAAR
    house_starts:       ds?.HOUST?.value ?? null,           // K SAAR
    building_permits:   ds?.PERMIT?.value ?? null,          // K SAAR
    case_shiller_yoy:   ds?.CSUSHPINSA?.value ?? null,     // % YoY (pc1 units applied)
    // ── P3: new FRED series (buildResearchIndicators Tier B) ──────────────────
    dgs1:                  research.dgs1 ?? null,
    dgs5:                  research.dgs5 ?? null,
    payems:                research.payems ?? null,
    breakeven_10y:         research.breakeven_10y ?? null,
    nfci:                  research.nfci ?? null,
    median_home_price_chg: research.median_home_price_chg ?? null,
    gold_price:            credit.gold_price ?? null,
    // ── P4: Core CPI YoY ──────────────────────────────────────────────────────
    core_cpi_yoy:          research.core_cpi_yoy ?? null,
    // ── Phase 5: TED spread (bps) + MOVE proxy (bps/year) ─────────────────────
    ted_spread,                                     // (SOFR+0.26161 − DTB3) × 100 bps
    move_index:            research.move_index ?? null, // std(22 DGS10 daily Δ) × √252 × 100
    // ── Phase 6: SKEW proxy from VIX ──────────────────────────────────────────
    skew_index,                                     // 100 + (VIX − 15) × 1.2
    // ── Dalio Long-Term Debt Cycle stage ──────────────────────────────────────
    dalio_stage,                                    // 1–5, computed from FRED proxies daily
  };
