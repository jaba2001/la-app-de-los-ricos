...
const IC_FLAGS = {
  ...
  A2_RECESSION_GATE: false,
};

// FRED fetch returning a short desc series [{date, v}, …]. Throws on HTTP/empty.
async function fredSeries(id, fredKey, limit) {
  const url = `https://api.stlouisfed.org/fred/series/observations?series_id=${id}&api_key=${fredKey}&file_type=json&sort_order=desc&limit=${limit}`;
  const r = await fetch(url);
...
async function buildResearchIndicators(fredKey) {
  ...
  const tryGet = async (id, limit) => {
    try { return await fredSeries(id, fredKey, limit); } catch { return null; }
  };
  const [dgs2, dgs10, dgs30, tp10, icsa, cp, walcl, rrp, tga, boj] = await Promise.all([
...
// BKLN = leveraged loans). FMP only; no key or any failure → nulls. Refreshed in
// the cron, never per user (0 new user quota). Divergence (HY tight but proxy
// weak) is finalised in buildMacroState where HY OAS is available.
async function buildCreditProxy(fmpKey) {
  if (!fmpKey) return { credit_private_proxy: null, credit_divergence: null };
  const q = async (sym) => {
    try {
      const r = await fetch(`https://financialmodelingprep.com/stable/quote?symbol=${sym}&apikey=${fmpKey}`);
      if (!r.ok) return null;
      const d = await r.json();
  ...
  const ch = [bizd?.changePercentage ?? bizd?.changesPercentage, bkln?.changePercentage ?? bkln?.changesPercentage]
...
async function buildMacroState(fredKey, opts = {}) {
  ...
  try { credit = await buildCreditProxy(opts.fmpKey); }
  ...
  if (IC_FLAGS.A2_RECESSION_GATE && reg && reg.id === "contraction" && research.recession_gate_active === true) {
    reg = classifyRegime(56, 56, 56); // the neutral/transición branch
  }
