...
//   (toma ic-proxy/.env.local automáticamente si existe; las env explícitas mandan)
//
// CONFIG (env vars, todas con default salvo los secretos):
//   PROXY_URL            (def. https://ic-proxy-psi.vercel.app)
...
// ── Cargar ic-proxy/.env.local (sin pisar lo que ya esté en el entorno) ──
...
const CFG = {
  PROXY_URL: (process.env.PROXY_URL || 'https://ic-proxy-psi.vercel.app').replace(/\/$/, ''),
