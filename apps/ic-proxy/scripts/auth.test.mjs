// Verificación local del token (lib/auth.js).
...
const { verifyTokenLocally, localVerifyAvailable } = await import('../lib/auth.js');
...
// ── Sin configuración: "no he podido comprobarlo", NO "invalido" ────────────────────
{
  ...
  const fresh = await import(`../lib/auth.js?nocfg=${Date.now()}`);
