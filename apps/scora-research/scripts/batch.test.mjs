...
import { route } from '../lib/server/providers/batch.js';
...
const batchSrc = readFileSync(new URL('../lib/server/providers/batch.js', import.meta.url), 'utf8');
const declared = {};
for (const m of batchSrc.matchAll(/limiter: '([a-z-]+)',\s*max: (\d+),\s*window: (\d+)/g)) {
  declared[m[1]] = { max: +m[2], window: +m[3] };
}
// Los limites viven en los modulos de proveedor, no en los route.js: estos son un
// re-export fino porque Next no deja exportar `serve` desde un fichero de ruta.
const ROUTES = {
  fmp:      'lib/server/providers/fmp.js',
  finnhub:  'lib/server/providers/finnhub.js',
  shortint: 'lib/server/providers/short-interest.js',
  finviz:   'lib/server/providers/finviz.js',
  edgar:    'lib/server/providers/edgar.js',
  simfin:   'lib/server/providers/simfin.js',
  congress: 'lib/server/providers/congress.js',
};
